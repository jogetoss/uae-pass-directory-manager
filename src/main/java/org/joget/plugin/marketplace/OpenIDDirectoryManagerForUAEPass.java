...
public class OpenIDDirectoryManagerForUAEPass extends SecureDirectoryManager {

    public SecureDirectoryManagerImpl dirManager;

    @Override
    public String getName() {
        return "OpenID Connect Directory Manager for UAEPass";
    }

    @Override
    public String getDescription() {
        return "Directory Manager with support for OpenID Connect for UAEPass";
    }
    ...
    public String getPropertyOptions() {
        ...
        callbackUrl += request.getContextPath() + "/web/json/plugin/org.joget.plugin.marketplace.OpenIDDirectoryManagerForUAEPass/service";
        String entityId = callbackUrl;

        String json = AppUtil.readPluginResource(getClass().getName(), "/properties/app/OpenIDDirectoryManagerForUAEPass.json", new String[]{callbackUrl, usJson, addOnJson}, true, "messages/open-id-authentication");
        return json;
    }
        ...
        try {
            if ("dmOptions".equals(action)) {
                ...
                if (authResp instanceof AuthenticationErrorResponse) {
                    ...
                    LogUtil.error(OpenIDDirectoryManagerForUAEPass.class.getName(), null, "Error During Authentication Response: " + error.toString());
                ...
                if (successResponse != null) {
                    if (!state.equals(successResponse.getState())) {
                        LogUtil.error(OpenIDDirectoryManagerForUAEPass.class.getName(), null, "Unexpected authentication response");
                        request.getSession().setAttribute("SPRING_SECURITY_LAST_EXCEPTION", "Error in Redirecting To External Login Page");
                        response.sendRedirect(url);
                    } else {
            ...
            LogUtil.error(OpenIDDirectoryManagerForUAEPass.class.getName(), ex, "Error in Redirecting To External Login Page");
            request.getSession().setAttribute("SPRING_SECURITY_LAST_EXCEPTION", "Error in Redirecting To External Login Page");
            response.sendRedirect(url);
        }
    }
    ...
    public OIDCProviderMetadata IssuerDiscovery() throws IOException, URISyntaxException {
        ...
        try {
            ...
            LogUtil.error(OpenIDDirectoryManagerForUAEPass.class.getName(), ex, "Error on Issuer Discovery");
            return null;
        }
    ...
    public URI getAuthorizationEndpointUri(Nonce nonce, State state) throws URISyntaxException, IOException {
        ...
        if(dmImpl.getPropertyString("issuer").equals("uaePass")) {
            String authorizationEndpoint = dmImpl.getPropertyString("authorizationEndpoint");
            String responseType = "?response_type=" + dmImpl.getPropertyString("responseTypes");
            String clientId = "&client_id=" + dmImpl.getPropertyString("clientId");
            String scope = "&scope=" + dmImpl.getPropertyString("scope");
            String uaeState = "&state=" + dmImpl.getPropertyString("state");
            String redirectURI = "&redirect_uri=" + getCallbackURL() + "&acr_values=urn:safelayer:tws:policies:authentication:level:low";
            URI uaeURI = new URI(authorizationEndpoint+responseType+clientId+scope+uaeState+redirectURI);
            return uaeURI;
        }
        ...
        if (dmImpl.getPropertyString("authorizationEndpoint").isEmpty() && !dmImpl.getPropertyString("authorizationEndpoint").equals("uaePass")) {
    ...
    public UserInfo getUserInfo(AccessToken accessTokenContent) throws URISyntaxException, IOException {
        ...
        try {
            ...
            LogUtil.error(OpenIDDirectoryManagerForUAEPass.class.getName(), ex, "Failed to parse userInfo Response");
            return null;
        }

        if (userInfoResponse != null) {
            if (!userInfoResponse.indicatesSuccess()) {
                LogUtil.error(OpenIDDirectoryManagerForUAEPass.class.getName(), null, "The request failed, e.g. due to invalid or expired token");
                return null;
            }
            // Extract the claims
    ...
    public AccessToken getTokenForCode(String code, Nonce nonce) throws URISyntaxException, IOException {
        ...
        try {
            ...
            LogUtil.error(OpenIDDirectoryManagerForUAEPass.class.getName(), ex, "Error Parsing Token Response");
            return null;
        }

        if (!tokenResponse.indicatesSuccess()) {
            TokenErrorResponse errorResponse = tokenResponse.toErrorResponse();
            LogUtil.error(OpenIDDirectoryManagerForUAEPass.class.getName(), null, "Error response:" + errorResponse.getErrorObject().toString() + " : " + errorResponse.getErrorObject().getDescription());
            return null;
        }
        ...
        if(dmImpl.getPropertyString("issuer").equals("uaePass")) {
            String originalString = dmImpl.getPropertyString("clientId")+":"+dmImpl.getPropertyString("clientSecret");
            ...
            URI tokenInspect = new URI(dmImpl.getPropertyString("tokenValidationEndpoint"));
            HttpPost httpPost = new HttpPost(tokenInspect);
            ...
            httpPost.addHeader("Authorization", encodedString);
            ...
            if(response.getStatusLine().getStatusCode() == 200) {
                ...
                LogUtil.error(OpenIDDirectoryManagerForUAEPass.class.getName(), null, "Invalid Token");
                return null;
            }
        ...
        try {
            ...
            LogUtil.error(OpenIDDirectoryManagerForUAEPass.class.getName(), e, "Invalid Claims");
            return null;
        }
        return accessToken;
    }
        ...
        try {
            ...
            if (user == null && userProvisioningEnabled) {
                ...
                if (userInfo.getGivenName() != null && !userInfo.getGivenName().isEmpty()) {
                    ...
                    if(userInfo.getClaim("firstnameEN") != null) {
                        user.setFirstName(userInfo.getClaim("firstnameEN").toString());
                    }
                }
                ...
                if (userInfo.getFamilyName() != null && !userInfo.getFamilyName().isEmpty()) {
                    ...
                    if(userInfo.getClaim("lastnameEN") != null) {
                        user.setLastName(userInfo.getClaim("lastnameEN").toString());
                    }
                }
